...
namespace Car_Rental_Service.Controllers
{
    ...
    public class CarRentedController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<Rented_Car>>> GetAllRentedcar()
        {
            var model = await _repository.SelectAll<Rented_Car>();
            return model;
        }
        ...
        public async Task<IActionResult> UpdateRentedCar(int id, [FromBody] Rented_CarVM model)
        {
            ...
            if (newmodel != null)
            {
                ...
                newmodel.Car_UIN = model.Car_UIN;
